...
test("It should record when a target connects", async () => {
  ...
  el.querySelectorAll("[flow-target~='example.item']").forEach((target, index) => {
...
test("It should not count nested targets", async () => {
  ...
  const controller = application.getController(el, "example")
  const nestedController = application.getController(el.querySelector("[flow-controller~='example']"), "example")

  assert.equal(controller.hasItemTarget, true)
  assert.equal(controller.itemTargets.length, 3)
  assert.equal(controller.itemTarget, el.querySelector("[flow-target]"))

  nestedController.element.querySelectorAll(".nested[flow-target~='example.item']").forEach((target, index) => {
...
test("It should not count nested targets when using multiple controllers", async () => {
  ...
  const nestedController = application.getController(el.querySelector("[flow-controller~='example-1']"), "example-1")
  ...
  nestedController.element.querySelectorAll(".nested[flow-target~='example-1.item']").forEach((target, index) => {
    assert.equal(target, nestedController.itemTargets[index])
  })
  ...
  el.querySelector("[flow-target]").setAttribute("flow-target", "example-2.item")
