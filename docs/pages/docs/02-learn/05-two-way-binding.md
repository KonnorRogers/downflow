...
Add `:context` after the key to send a binding somewhere other than the ambient `flow-context`, without changing the attribute on a parent element.

```html
<div flow-controller="settings" flow-context="settings">
  <!-- still goes to settings' context -->
  <input flow-bind="theme" />
  <!-- opts out, writes to application.context instead -->
